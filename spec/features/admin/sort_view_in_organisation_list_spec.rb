  ...
  context 'when admin views the table with organisation data' do
    ...
    context 'and wants to sort by MoU uploads' do
      before do 
        ...
        create(:organisation, name: "Has New MoU Company").signed_mou.attach(
          io: File.open(Rails.root + "spec/fixtures/mou.pdf"), filename: "mou.pdf"
        )

        create(:organisation, name: "Has Old MoU Company").signed_mou.attach(
          io: File.open(Rails.root + "spec/fixtures/mou.pdf"), filename: "mou.pdf"
        )

        Organisation.find_by(name: "Has New MoU Company").signed_mou_attachment.update(created_at: 3.years.from_now)
      end

      it 'sorts the organistions from oldest to newest when MoU signed is clicked once' do
        click_link 'MoU Signed'

        expect(page.body).to match(/No MoU.*Old MoU.*New MoU/m)
      end

      it 'sorts the organistions from newest to oldest when MoU signed is clicked twice' do
        2.times { click_link 'MoU Signed' }

        expect(page.body).to match(/New MoU.*Old MoU.*No MoU/m)
      end
    end
  end
end
