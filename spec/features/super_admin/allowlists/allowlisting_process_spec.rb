describe "Allowlisting an organisation", type: :feature do
  before do
    sign_in_user create(:user, :super_admin)
    visit new_super_admin_allowlist_path
  end
  ...
  it "allows the user to see the list of allowlisted organisations" do
    click_on "Organisations allow list"
    expect(page).to have_content("Custom Organisations that are already in our register")
  end

  it "allows the user to see the list of allowlisted email domains" do
  ...
  context "when validating the form input" do
    context "with an incorrect organisation name" do
      ...
      before do
        ...
        visit new_super_admin_allowlist_path(allowlist: { step: Allowlist::FOURTH })
