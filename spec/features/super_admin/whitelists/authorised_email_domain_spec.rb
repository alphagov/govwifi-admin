  ...
  context "when whitelisting a domain" do
    ...
    context "when deleting a whitelisted domain" do
      ...
      it "removes a domain" do
        expect { click_on "Yes, remove #{some_domain} from the allowed list" }.to change(AuthorisedEmailDomain, :count).by(-1)
      end

      it "tells the user the domain has been removed" do
        click_on "Yes, remove #{some_domain} from the allowed list"
