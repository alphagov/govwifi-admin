...
class Location < ApplicationRecord
  ...
  def sibling_locations
    organisation.locations
  end

  def unpersisted_addresses_are_unique
    return if persisted? || organisation.nil?
    all_location_addresses = sibling_locations.reject(&:persisted?).pluck(:address)
    errors.add(:address, :unpersisted_duplicate, address: address) if all_location_addresses.count(address) > 1
  end
