  ...
  try {
    timeout(time: 5, unit: 'MINUTES') {
      ...
      docker.withRegistry(env.AWS_ECS_API_REGISTRY) {
        ...
        sh("bin/ecs-deploy --cluster ${deploy_environment}-admin-cluster --task-definition  admin-task-${deploy_environment}  --use-latest-task-def --desired-count ${desired_count} --image govwifi/admin:${deploy_environment} --timeout 300 --verbose")
      }
    }
